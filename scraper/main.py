...
# if WEB_DRIVER env variable is not set
path_webdriver = os.environ.get('WEB_DRIVER')  # returns None if env var not set
# check operating system to use web drivers in proj folder if WEB_DRIVER not set
if not path_webdriver:
    print('WEB_DRIVER not set, using drivers in proj folder.')
    # checking the OS app is running on
    if os.name == 'nt':
        # running on windows, use driver for windows
        path_webdriver = os.path.join(path_module, 'chromedriver.exe')
    else:
        # can be 'java' or 'posix', but I only use posix so not checking for java here
        path_webdriver = os.path.join(path_module, 'chromedriver2.44.exe')
else:
    print('Using WEB_DRIVER environment variable path...')
    print('WEB_DRIVER={0}'.format(path_webdriver))


def scrape_movies(infos):
    movies = []
    for info in infos:
        title = info.h2.string
        spans = info(name='span', class_='cgv-info-normal')
        # strip=True - stripping whitespaces from beginning and end of result
...
def get_chrome_webdriver():
    ...
    # open browser
    browser = webdriver.Chrome(executable_path=path_webdriver,
                               chrome_options=chrome_options)
    return browser


def get_phantom_webdriver():
